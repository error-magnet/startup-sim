...
function NegotiationModal({ employee, dispatch, sym }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div className="t-bg-card t-border border p-5 max-w-sm w-[calc(100%-2rem)] mx-4 sm:mx-0 flex flex-col gap-3">
  ...
  return (
    <div className="h-screen flex flex-col t-bg t-text overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center justify-between sm:justify-start gap-0 overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0">
          <div className="flex gap-0 shrink-0">
            {TABS.map((tab) => (
              <button
                key={tab.key}
                onClick={() => dispatch({ type: 'SET_TAB', tab: tab.key })}
                className={`px-3 py-1 text-sm t-border border -ml-px first:ml-0 transition-colors whitespace-nowrap ${
      ...
      {state.gameOver && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.7)' }}>
          <div className="t-bg-card t-border border p-6 max-w-sm w-[calc(100%-2rem)] mx-4 sm:mx-0 text-center flex flex-col gap-3">
